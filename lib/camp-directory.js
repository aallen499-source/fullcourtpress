...
import { isListable } from './showcases';
...
export async function getCamps(supabase, sportSlug, stateCode) {
  ...
  const { data } = await supabase
    .from('camps')
    .select('school, camp_name, division, cost, eligibility, city, state, date, type, source_url, sport, attending_programs')
    .eq('state', stateCode)
    .gte('date', start)
    .order('date');
  const all = (data || []).filter((c) => sportKey(c.sport) === sportSlug && isListable(c));
...
export async function campIndex(supabase) {
  ...
  const { data } = await supabase
    .from('camps')
    .select('sport, state, date, type, attending_programs')
    .gte('date', start);
  const map = new Map();
  for (const c of data || []) {
    if (!isListable(c)) continue;
