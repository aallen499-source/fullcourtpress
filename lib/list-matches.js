...
const ALIASES = {
  ...
  oregontech: 'oregoninstitutetechnology',
 ...
 * @param athleteGender 'men' | 'women' | '' — used when a roster row names no gender
 * @returns [{ camp, kind: 'camp' | 'showcase', schools: [{ name, tier }] }]
 */
export function listMatches(coaches, camps, profileSport = '', athleteGender = '') {
  ...
  for (const c of coaches || []) {
    ...
    listed.set(key, {
      ...
      gender: prev?.gender || genderOfText(c.sport) || athleteGender,
    });
  }
...
/**
 * 'men' | 'women' | '' from the catalogue camps an athlete tracks. No gender is
 * stored for an athlete, and the roster's sport says "Basketball", so the camps
 * they chose are the honest signal. Only a clear majority counts.
 *
 * @param trackedIds Set of camps.id the athlete tracks
 * @param camps      catalogue rows ({ id, sport })
 */
export function athleteGenderFrom(trackedIds, camps) {
  const tally = { men: 0, women: 0 };
  for (const c of camps || []) {
    if (!trackedIds.has(c.id)) continue;
    const g = String(c.sport || '').split('-')[1];
