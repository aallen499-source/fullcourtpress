...
{{your_name}}`,
  ...
  {
    // The short introduction with two sentences the athlete writes: who they
    // are, and how they'd describe their game. Still front-loads the vitals and
    // the film link — a coach reading on a phone decides on those — but it
    // reads like a person rather than a form. For Dream and Target schools,
    // where the extra two minutes are worth spending; the short one stays for
    // writing to many schools at once.
    id: 't_intro_long',
    name: 'Initial Introduction (longer)',
    subject: '{{tagline}} | {{location}}',
    body: `Hi Coach {{coach_last}},
...
/**
 * The athlete's next few dates, as lines for the "Invite to a Game" email.
 *
 * Falls back to the bracketed prompt the template used to carry, because a
 * coach reading "My next games:" followed by nothing is worse than a line the
 * athlete is obviously meant to fill in — the composer highlights brackets for
 * exactly that reason.
 */
...
function scheduleLines(events) {
  ...
  if (!rows.length) return SCHEDULE_PROMPT;
  return rows
    .map((e) => {
      const [y, m, d] = String(e.date || '').split('-').map(Number);
      if (!y) return '';
      const start = new Date(y, m - 1, d);
      const day = (dt) => dt.toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' });
      let when = day(start);
      if (e.end_date && e.end_date !== e.date) {
        const [ey, em, ed] = String(e.end_date).split('-').map(Number);
        if (ey) when = `${day(start)}–${new Date(ey, em - 1, ed).getDate()}`;
      }
      return join([when, e.time_note, e.title, e.location], ' · ');
    })
    .filter(Boolean)
    .join('\n');
}
...
export function fillMergeTags(str, coach, profile) {
  ...
  let text = str || '';
  // The athlete's phone goes under their name in the sign-off — on every
  // template, saved ones included, without anyone editing them. Only the last
  // {{your_name}} sitting alone on its own line (the signature), and only if
  // the template doesn't already place {{phone}} itself. A subject is one line,
  // so a subject that starts with the name ("{{your_name}} — Class of 2028")
  // is never a signature — it used to get the phone number glued on.
  if (profile?.phone && !text.includes('{{phone}}') && text.includes('\n')) {
    const at = text.lastIndexOf('{{your_name}}');
    const end = at + '{{your_name}}'.length;
    const startsLine = at === 0 || text[at - 1] === '\n';
    // A template like Jamario's "{{your_name}}{{profile_link}}" still counts:
    // the link is moved onto its own line below.
    const endsLine = end === text.length || text[end] === '\n' || text.startsWith('{{profile_link}}', end);
    if (at >= 0 && startsLine && endsLine) {
      text = `${text.slice(0, at)}{{your_name}}\n{{phone}}${text.slice(at + '{{your_name}}'.length)}`;
    }
  }
  ...
  return tidy(text
    ...
    .replaceAll('{{schedule}}', scheduleLines(profile?.events))
