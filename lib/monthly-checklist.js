...
// checklistAutoDone below), months (only in those months, 0 = January), batch
// (a template id — the item opens "write to your coaches" with that template).
const PLAN = {
  middle: {
    all: {
  ...
  sophomore: {
    fall: {
      ...
      items: [
        ...
        { id: 'intro', label: 'Email an introduction to your Target schools', hint: 'Templates → Initial Introduction. Three this month is a good start.', tab: 'templates', auto: 'emailed3' , batch: 't_intro' },
    ...
    winter: {
      ...
      items: [
        { id: 'update', label: 'Send a season update with new film', hint: 'Templates → Season / Film Update, to every coach you have emailed.', tab: 'templates', auto: 'emailed3' , batch: 't_film_update' },
    ...
    summer: {
      ...
      items: [
        ...
        { id: 'before', label: 'Tell coaches which camps and events you’ll be at', hint: 'Templates → Before a Camp or Showcase.', tab: 'templates', auto: 'emailed3' , batch: 't_before_camp' },
  ...
  junior: {
    fall: {
      ...
      items: [
        { id: 'update', label: 'Email your Target coaches a junior-year update', hint: 'Grades, schedule and newest film. Aim for five coaches this month.', tab: 'templates', auto: 'emailed5' , batch: 't_film_update' },
        ...
        { id: 'schedule', label: 'Send your season schedule so coaches can come watch', hint: 'Templates → Invite to a Game.', tab: 'templates', batch: 't_game_invite' },
      ],
    },
    winter: {
      intro: 'In season. Keep the coaches who answered close, and keep the list honest.',
      items: [
        { id: 'film', label: 'Send new film to every coach who has replied', hint: 'A short update every few weeks is welcome, not pushy.', tab: 'templates', auto: 'emailed3' , batch: 't_film_update' },
        { id: 'quiet', label: 'Follow up with anyone quiet for three weeks or more', hint: 'Templates → Follow-up.', tab: 'templates' , batch: 't_followup' },
    ...
    spring: {
      ...
      items: [
        ...
        { id: 'call', label: 'Ask the coaches you talk to most for a call or visit', hint: 'Templates → Requesting a Visit or Call.', tab: 'templates' , batch: 't_visit_request' },
      ],
    },
    summer: {
      intro: 'Get seen, then follow up. The follow-up is where most athletes drop off.',
      items: [
        { id: 'before', label: 'Tell coaches where you’ll play before each event', hint: 'Templates → Before a Camp or Showcase.', tab: 'templates', auto: 'emailed3' , batch: 't_before_camp' },
  ...
  senior: {
    fall: {
      ...
      items: [
        ...
        { id: 'keep', label: 'Keep emailing — D2, D3, NAIA and junior college coaches recruit into spring', hint: 'Templates → Season / Film Update.', tab: 'templates', auto: 'emailed3' , batch: 't_film_update' },
      ],
    },
    winter: {
      ...
      items: [
        ...
        { id: 'update', label: 'Send senior season film to every coach still in the picture', hint: 'Templates → Season / Film Update.', tab: 'templates', auto: 'emailed3' , batch: 't_film_update' },
...
/**
 * When a coach was last written to. last_emailed_at is set on every email sent
 * from RecruitGrid; status_changed_at only moves the first time, so it alone
 * would miss every follow-up. A status set by hand with no email logged falls
 * back to it.
 */
export function lastContactAt(c) {
  const candidates = [c?.last_emailed_at, CONTACTED(c?.status) ? c?.status_changed_at : null].filter(Boolean);
...
export function checklistAutoDone({ coaches = [], film = [], camps = [] }, now = new Date(), timeZone) {
  ...
  const emailedThisMonth = coaches.filter((c) => inThisMonth(lastContactAt(c))).length;
  return {
    film: film.length > 0,
