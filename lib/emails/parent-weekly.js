...
//
// The layout helpers are exported for the other account emails (open alerts,
// announcements) so they all look like one product.
//
...
export function parentWeeklyEmail(w, stopUrl) {
  ...
  if (w.dueSteps?.length) {
    parts.push(section(label('📅 Coming due') + rows(w.dueSteps.map((d) => ({
      title: `${esc(d.note)} · ${esc(d.school)}${tierTag(d.tier)}`,
      meta: esc(d.meta),
    })))));
  }
