...
export function openAlertEmail({ firstName, coachLabel, school, openCount, lastEmailed, coachId, unsubscribeUrl }) {
  ...
  const body = [
    ...
    `<tr><td style="padding:22px 32px 2px;">
       <p style="margin:0 0 12px;font:400 15px/1.6 ${FONT};color:${BODY};">
         ${firstName ? `${esc(firstName)}, the` : 'The'} recruiting profile link in your email was opened. ${esc(facts)}
       </p>
  ...
  const footer = `You're getting this because you sent this coach your RecruitGrid profile link. <a href="${esc(unsubscribeUrl)}" style="color:${MUTED};text-decoration:underline;">Turn off these alerts</a>`;
