...
import { monthlyChecklist, checklistAutoDone, resolveChecklist, ymdIn, lastContactAt } from './monthly-checklist';
...
export function composeParentWeekly(data, now = new Date()) {
  ...
  // ---- the week in numbers
  const contactedThisWeek = coaches.filter((c) => inWeek(lastContactAt(c)));
  ...
  const opens = openedThisWeek.slice(0, 5).map((c) => {
    ...
    const contact = last ? ` · ${first} last contacted them ${shortDate(last)}` : '';
    return {
  ...
  const nudges = coaches
    ...
    .filter((c) => {
      const last = lastContactAt(c);
      return !last || new Date(last).getTime() < now.getTime() - NUDGE_DAYS * DAY;
    })
    ...
    .map((c) => ({
      school: c.school,
      tier: c.tier,
      meta: lastContactAt(c) ? `Last contacted ${shortDate(lastContactAt(c))}` : 'Not contacted yet',
    }));
  ...
  // ---- next steps set on coaches, due in the coming week or already past
  const weekOut = ymdIn(new Date(now.getTime() + 7 * DAY), PARENT_TZ);
  const dueSteps = coaches
    .filter((c) => c.next_step_on && c.next_step_on <= weekOut)
    .sort((a, b) => a.next_step_on.localeCompare(b.next_step_on))
    .slice(0, 5)
    .map((c) => ({
      school: c.school || '',
      tier: c.tier || '',
      note: c.next_step_note || 'Follow up',
      meta: c.next_step_on < today ? `Was due ${plainDate(c.next_step_on)}` : c.next_step_on === today ? 'Due today' : `Due ${plainDate(c.next_step_on)}`,
    }));
  ...
  const openNoFollowUp = openedThisWeek.find(
    (c) => !lastContactAt(c) || new Date(lastContactAt(c)) < new Date(c.link_last_opened_at)
  );
  const soon = upcoming.find((u) => u.date <= ymdIn(new Date(now.getTime() + 14 * DAY), PARENT_TZ));
  const lateStep = dueSteps.find((d) => d.meta.startsWith('Was due') || d.meta === 'Due today');
  if (lateStep) {
    question = `“${lateStep.note}” for ${lateStep.school} ${lateStep.meta === 'Due today' ? 'is due today' : lateStep.meta.toLowerCase()}. Has it been done?`;
  } else if (openNoFollowUp) {
